...
def make_maskAndSave(img_path, predict, masked_size, sigma, rand_times, threshold, path=""):
    ...
    """
    txt_root = path + '/txt_' + str(masked_size[0]) + 'x' + str(masked_size[1]) + '_' + str(sigma) + '_' + str(threshold) + '/'
    maskedImg_root = [path + '/masked_img/' + str(masked_size[0]) + 'x' + str(masked_size[1]) + '_' +
                      str(sigma) + '_' + str(threshold)+ '/-1/', path + '/masked_img/' + str(masked_size[0]) + 'x' + str(masked_size[1]) + '_' +
                      str(sigma) + '_' + str(threshold)+ '/1/']
    ...
    for time in range(1, rand_times+1):
        ...
        if abs(res[0] - initial_res[0]) > threshold:
            ...
            if initial_res[0] > 0.5:
                f.write(str(res[0].item()) + ' ' + str(res[1].item()) + ' ' + str(mask_x) + ' ' + str(mask_y) + ' -1' + '\n')
        ...
        #     masked_image.save(maskedImg_root + os.path.splitext(os.path.basename(img_path))[0] + '_' + str(time) + '.jpg')
                masked_imageCrop.save(maskedImg_root[0] + os.path.splitext(os.path.basename(img_path))[0] + '_' + str(time) + '.tif')
            else:
                f.write(str(res[0].item()) + ' ' + str(res[1].item()) + ' ' + str(mask_x) + ' ' + str(
                    mask_y) + ' 1' + '\n')
                masked_imageCrop.save(
                    maskedImg_root[1] + os.path.splitext(os.path.basename(img_path))[0] + '_' + str(time) + '.jpg')
    f.close()
...
if __name__ == '__main__':
    """
        保存两个文件夹：1、原图小块+sigma+threshold； 2、概率信息和原图小块中心
    """
    img_root = "E:/GangLiu\AI\data/20230811Test/test/"
    masked_size = [(60, 60)]
    sigma = 20
    random_times = 50
    threshold = [0.5, 0.7, 0.9]
    work_path = "E:/GangLiu/AI/data/20230811Test"
    ...
    weights_path = '../models/0730Adamlr1e-5/GoogleNet_batch_size128_100e_lr1e-05_adam.pth'
        ...
        for th in threshold:
            ...
            for label in labels:
                ...
                for img_name in img_names:
                    ...
                    make_maskAndSave(img_path, predict, ms, sigma, random_times, threshold=th, path=work_path)

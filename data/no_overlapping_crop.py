...
if __name__ == '__main__':
    ...
    n = int(img_x * img_y / target_size / target_size)
    prop = [0.8 * n, 0.9 * n, 1 * n]
    X = [i for i in range(0, img_x, target_size)]
    Y = [i for i in range(0, img_y, target_size)]

    for img_root in img_r:
        Train_path = "nolpdata/" + "train" + '/' + img_root.split('/')[3]
        Test_path = "nolpata/" + "test" + '/' + img_root.split('/')[3]
        Val_path = "nolpdata/" + "val" + '/' + img_root.split('/')[3]
        ...
        for img_r in os.listdir(img_root):
            ...
            for x in X:
                for y in Y:
                    if judge_RD(x, y) and judge_bound(x, y, x_bound=1600, y_bound=1200):
                        crop_image = Custom_crop(image, x, y)
                        count += 1
            ...
            c.add(count)
    print("each image had generated {:d} subimages".format(c.pop()))
    print("done!")